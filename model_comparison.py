...
random_model = RandomForestClassifier(criterion="entropy")
...
support_model = SVC(kernel="rbf", C=10.0)

support_model.fit(x_train, y_train)
y_pred = support_model.predict(x_test)

s_accuracy = support_model.score(x_test, y_test)
y_pred = support_model.predict(x_test)
cm = confusion_matrix(y_test, y_pred)
...
matplt.bar(
    ...
    [d_accuracy, r_accuracy, s_accuracy, mlp_accuracy],
)
